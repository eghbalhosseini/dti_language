...
for k=1:length(seeds)
    ...
    for kk=1:length(targets)
        ...
        s_t_con=cat(3,results{t_idx}.unique_LH_fdt_sum{:});
        assert(sum(sum(contains(results{t_idx}.unique_LH_targets,targets{kk})))==size(s_t_con,3));
        assert(sum(sum(contains(results{t_idx}.unique_LH_targets,seeds{k})))==size(s_t_con,3));
