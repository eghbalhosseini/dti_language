...
%ff=figure('Position', [100, 100, 800, 600]);
ff=figure();
ff.Units='Inches';
...
pa_ratio=8/11;
factor=21/29.7 ;
ax1=axes('position',[.5,.4,.25,.25*pa_ratio]);
% Create a bar graph with customized colors
b = barh(means, 'FaceColor', 'flat', 'EdgeColor', 'k');
for k = 1:numSubjects
    b.CData(k,:) = cmap(3,:);
end
hold on;
...
errorbar( means,1:numSubjects, stdDevs/sqrt(size(sub_vox_count,2)),'horizontal', ...
'linestyle','none','color','k','linewidth',2,'capsize',0);

% Customize the axes
xlabel({'Mean and Standard Deviation of','Percentages for Each Subject'}, 'FontSize',12, 'FontWeight', 'normal');


% Customize the x-axis labels
yticks(1:numSubjects);
yticklabels(arrayfun(@(x) trac_names{x}, 1:numSubjects, 'UniformOutput', false));
...
% for k = 1:numSubjects
%     text( means(k) + stdDevs(k)/sqrt(size(sub_vox_count,2)) + 1,k, ...
%         sprintf('%.1f', means(k)), ...
%         'HorizontalAlignment', 'center', 'FontSize', 12, 'FontWeight', 'bold');
% end

ax1=makeaxis_eh(ax1);
ax1.Title.String=sprintf('%s\n', fig_title);
ax1.Title.FontSize=10;
ax1.Box='off';
% Optional: Add a colorbar if colors represent specific information
% colorbar;

%export_fig(sprintf('/Users/eghbalhosseini/MyData/dti_language/tracula/probtrac_by_tracula_%s_nsub_%d_20250517.pdf',fig_title,sub_id), '-pdf', '-transparent');
print(ff,'-dpdf','-bestfit','-painters', sprintf('/Users/eghbalhosseini/MyData/dti_language/tracula/probtrac_by_tracula_%s_nsub_%d_20250517.pdf',fig_title,sub_id));
print(ff,'-painters','-dpng', sprintf('/Users/eghbalhosseini/MyData/dti_language/tracula/probtrac_by_tracula_%s_nsub_%d_20250517.png',fig_title,sub_id));