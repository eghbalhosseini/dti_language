...
seeds={sprintf('RH_AntTemp_top_%d',threshold),sprintf('RH_PostTemp_top_%d',threshold),sprintf('RH_AngG_top_%d',threshold)};
%seeds={sprintf('RH_AntTemp_top_%d',threshold),sprintf('RH_PostTemp_top_%d',threshold)};
targets={sprintf('RH_IFG_top_%d',threshold);sprintf('RH_IFGorb_top_%d',threshold);sprintf('RH_MFG_top_%d',threshold)};
...
for k=1:length(seeds)
    ...
    [C,~,L_T]=intersect(seeds{k},train_dti.unique_RH_targets,'stable');
    [C,~,L_S]=intersect(targets,train_dti.unique_RH_targets,'stable');
    RH_fdt=train_dti.unique_RH_fdt_sum;
    % 
    target_fdt=cellfun(@(t) t(:,L_T),RH_fdt,'uni',false);
    ...
    for kk=1:length(targets)
        t_idx=contains(folders,erase([targets{kk},'-',seeds{k}],'RH_'));
        ...
        not_contains=~cell2mat(cellfun(@(x) contains(grp_fold,erase(x,'RH_')),remain,'uni',false));
        g_t_idx=find(contains(grp_fold,erase(seeds{k},'RH_')).*contains(grp_fold,erase(targets{kk},'RH_')).*not_contains);
        s_t_con=cat(3,results{t_idx}.unique_RH_fdt_sum{ids});
        %s_t_connect=results_path{t_idx}.('RH_path');
        %s_t_connect=cellfun(@(x) double(x./max(x)) ,s_t_connect(:,1),'uni',false);
        %s_t_connectivity=[s_t_connectivity,s_t_connect];
        %s_t_con_all=cat(3,group_results{g_t_idx}.unique_RH_fdt_sum{ids});
...
for k=1:length(targets)
    ...
    s_t_weights=[];
    [C,~,L_T]=intersect(targets{k},train_dti.unique_RH_targets,'stable');
    RH_fdt=train_dti.unique_RH_fdt_sum;
    % 
    target_fdt=cellfun(@(t) t(:,L_T),RH_fdt,'uni',false);
    target_sum=cellfun(@sum,target_fdt);
    for kk=1:length(seeds)
        t_idx=contains(folders,erase([targets{k},'-',seeds{kk}],'RH_'));
        s_t_con=cat(3,results{t_idx}.unique_RH_fdt_sum{ids});
        assert(sum(sum(contains(results{t_idx}.unique_RH_targets(:,ids),targets{k})))==size(s_t_con,3));
        assert(sum(sum(contains(results{t_idx}.unique_RH_targets(:,ids),seeds{kk})))==size(s_t_con,3));
        s_t_weights=[s_t_weights,squeeze([s_t_con(1,2,:)])];
...
%% 
print(ff,'-dpdf','-bestfit','-painters', strcat(analysis_path,'/','figure_3_RH_temporal_to_frontal_connectivity_pattern_thr_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.pdf'));
print(ff,'-painters','-dpng', strcat(analysis_path,'/','figure_3_temporal_RH_to_frontal_connectivity_pattern_thr_','_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.png'));
%% 
%%  
ff=figure();
ff.Units='Inches';
...
for k=1:length(seeds)
    ...
    for kk=1:length(targets)
        ...
        remain=setdiff(seeds,seeds(k));
        grp_fold=cellfun(@(x) x(1:regexp(x,'_EX_')), group_folders,'uni',false);
        not_contains=~sum(cell2mat(cellfun(@(x) contains(grp_fold,erase(x,'RH_')),remain,'uni',false)),2);
        g_t_idx=find(contains(grp_fold,erase(seeds{k},'RH_')).*contains(grp_fold,erase(targets{kk},'RH_')).*not_contains);
        %disp(group_folders(g_t_idx))
        
        s_t_con=cat(3,results{t_idx}.unique_RH_fdt_sum{ids});
        [C,~,L_T]=intersect(seeds{k},group_results{g_t_idx}.unique_RH_targets,'stable');
        s_t_con_all=cat(3,group_results{g_t_idx}.unique_RH_fdt_sum{ids});
        assert(sum(sum(contains(results{t_idx}.unique_RH_targets(:,ids),targets{kk})))==size(s_t_con,3));
        assert(sum(sum(contains(results{t_idx}.unique_RH_targets(:,ids),seeds{k})))==size(s_t_con,3));
        %s_t_connect=results_path{t_idx}.('RH_path');
...
for k=1:length(targets)
    ...
    for kk=1:length(seeds)
        ...
        grp_fold=cellfun(@(x) x(1:regexp(x,'_EX_')), group_folders,'uni',false);
        not_contains=~sum(cell2mat(cellfun(@(x) contains(grp_fold,erase(x,'RH_')),remain,'uni',false)),2);
        g_t_idx=find(contains(grp_fold,erase(seeds{kk},'RH_')).*contains(grp_fold,erase(targets{k},'RH_')).*not_contains);
        assert(length(g_t_idx)==1);
        disp(group_folders(g_t_idx))
        s_t_con=cat(3,results{t_idx}.unique_RH_fdt_sum{ids});
        [C,~,L_T]=intersect(targets{k},group_results{g_t_idx}.unique_RH_targets,'stable');
        assert(sum(sum(contains(results{t_idx}.unique_RH_targets(:,ids),targets{k})))==size(s_t_con,3));
        assert(sum(sum(contains(results{t_idx}.unique_RH_targets(:,ids),seeds{kk})))==size(s_t_con,3));
        s_t_weights=[s_t_weights,squeeze([s_t_con(1,2,:)])];
        s_t_weight_all=[s_t_weight_all,squeeze(sum(s_t_con_all(L_T,:,:),2))];
...
%% 
print(ff,'-dpdf','-bestfit','-painters', strcat(analysis_path,'/','figure_3_RH_temporal_to_frontal_con_ratio_thr_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.pdf'));
print(ff,'-painters','-dpng', strcat(analysis_path,'/','figure_3_RH_temporal_to_frontal_connectivity_ratio_thr_','_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.png'));
...
[C,~,LT]=cellfun(@(x) intersect(x,train_dti.unique_RH_targets,'stable'),all_targets);
% reformat areas 
RH_fdt=train_dti.unique_RH_fdt_sum;
target_fdt=cellfun(@(t) t(:,LT),RH_fdt,'uni',false);
...
print(ff,'-dpdf','-bestfit','-painters', strcat(analysis_path,'/','figure_3_RH_all_lang_con_ratio_thr_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.pdf'));
print(ff,'-painters','-dpng', strcat(analysis_path,'/','figure_3_RH_all_lang_connectivity_ratio_thr_','_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.png'));
%% 
...
print(ff,'-dpdf','-bestfit','-painters', strcat(analysis_path,'/','figure_3_RH_global_lang_con_ratio_thr_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.pdf'));
print(ff,'-painters','-dpng', strcat(analysis_path,'/','figure_3_RH_global_lang_connectivity_ratio_thr_','_',num2str(threshold),'_',w_type,'_subs_',num2str(length(train_subs)),'.png'));
