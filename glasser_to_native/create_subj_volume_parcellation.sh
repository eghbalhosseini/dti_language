...
# usage:
# bash create_subj_volume_parcellation -L <subject_list> -a <name_of_annot_file>
# -f <first_subject_row> -l <last_subject_row> -d <name_of_output_dir>
# 
# 
# HOW TO USE
# 
# Ingredients:
# 
# Subject data. First of all, you need to have your subjects’ structural data
# preprocessed with FreeSurfer.  Shell script. Download the script:
# create_subj_volume_parcellation, unzip it (because wordpress won’t upload .sh
# files directly), and copy it to to your $SUBJECTS_DIR/ folder.  Fsaverage
# data. Copy the fsaverage folder from the FreeSurfer directory
# ($FREESURFER_HOME/subjects/fsaverage) to your $SUBJECTS_DIR/ folder.
# Annotation files. Download rh.HCPMMP1.annot and lh.HCPMMP1.annot from
# https://figshare.com/articles/HCP-MMP1_0_projected_on_fsaverage/3498446. Copy
# them to your $SUBJECTS_DIR/ folder or to $SUBJECTS_DIR/fsaverage/label/.
# Subject list. Create a list with the identifiers of the desired target
# subjects (named exactly as their corresponding names in $SUBJECTS_DIR/, of
# course).
#  
# Instructions:
# 
# Launch the script: bash create_subj_volume_parcellation.sh (this will show the
# compulsory and optional arguments).  The compulsory arguments are: -L
# subject_list_name -a name_of_annotation_file (without hemisphere or extension;
# in this case, HCPMMP1) -d name_of_output_dir (will be created in
# $SUBJECTS_DIR) Optional arguments: -f and -l indicate the first and last
# subjects in the subject list to be processed. Eg, in order to process the
# third till the fifth subject, one would enter -f 3 -l 5 (whole thing takes a
# bit of time, so one might want to launch it in separate terminals for speed)
# -m (YES or NO, default NO) indicates whether individual volume files for each
# parcellation region should be created. This requires FSL -s (“YES” or “NO”,
# default is NO) indicates whether individual volume files for each subcortical
# aseg region should be created. Also requires FSL, and requires that the
# FreeSurferColorLUT.txt file be present at the base (subjects) folder -t (YES
# or NO, default YES) indicates whether to create anatomical stats table (number
# of vertices, area, volume, mean thickness, etc.) per region Output: An output
# folder named as specified with the -d option will be created, which will
# contain a directory called label/, where the labels for the regions projected
# on fsaverage will be stored. The output directory will also contain a folder
# for each subject. Inside these subject folders, a .nii.gz file named as # the
# annotation file (-a option) will contain the final parcellation volume. A
...
# subjects’ label folders, post-transformation annotation files will be created.
# These are not overwritten if the script is relaunched; so, if you ran into a
# problem and want to start over, you should delete these files (named
# lh(rh).<subject>_<name_of_annotation_file>.annot)
