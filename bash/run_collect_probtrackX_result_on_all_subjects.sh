...
printf "%s,%s,%s,%s\n" "row" "subject_name" "hemi" "file_loc"   >> $SUBJECT_PROBX_FILE
...
while read x; do
      ...
      then
        ...
        printf "%d,%s,%s,%s\n" "$LINE_COUNT" "$subject_name" "LH" "$lh_tr_file" >> $SUBJECT_PROBX_FILE
      ...
      then
        ...
        printf "%d,%s,%s,%s\n" "$LINE_COUNT" "$subject_name" "RH" "$rh_tr_file" >> $SUBJECT_PROBX_FILE
      fi
