...
#threshold=20
probtrackX_labels_="all_subject_probtrackX_lang_tracts_thr_${threshold}"
LINE_COUNT=0
#####################################
...
if [ "$LINE_COUNT" -gt "$run_val" ]; then
  ...
  if [ "$LINE_COUNT" -lt 300 ] ; then
    echo "less than 300 jobs:  ${LINE_COUNT} jobs"
      nohup /cm/shared/admin/bin/submit-many-jobs $LINE_COUNT "$LINE_COUNT" "$LINE_COUNT" 0 probtrackX_on_lang_tracts.sh  $SUBJECT_PROBX_FILE
  else
    echo "more than 300 jobs:  ${LINE_COUNT} jobs"
      ...
      nohup /cm/shared/admin/bin/submit-many-jobs $LINE_COUNT 275 300 25 probtrackX_on_lang_tracts.sh  $SUBJECT_PROBX_FILE
  fi
  else
    echo $LINE_COUNT
fi